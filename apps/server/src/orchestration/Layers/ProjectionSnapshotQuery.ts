...
import { ProjectionThreadTaskPlan } from "../../persistence/Services/ProjectionThreadTaskPlans.ts";
...
const ProjectionThreadTaskPlanDbRowSchema = ProjectionThreadTaskPlan.mapFields(
  Struct.assign({
...
const ProjectionThreadActivityMetadataDbRowSchema = Schema.Struct({
...
type ProjectionThreadActivityMetadataRow = Schema.Schema.Type<
  typeof ProjectionThreadActivityMetadataDbRowSchema
>;
type ProjectionThreadProposedPlanRow = Schema.Schema.Type<
  typeof ProjectionThreadProposedPlanDbRowSchema
>;
type ProjectionThreadTaskPlanRow = Schema.Schema.Type<typeof ProjectionThreadTaskPlanDbRowSchema>;
...
const REQUIRED_SNAPSHOT_PROJECTORS = [
  ...
  ORCHESTRATION_PROJECTOR_NAMES.threadTaskPlans,
...
function mapProposedPlanRow(row: ProjectionThreadProposedPlanRow): OrchestrationProposedPlan {
  return {
    id: row.planId,
    turnId: row.turnId,
...
function mapTaskPlanRow(row: ProjectionThreadTaskPlanRow): OrchestrationTaskPlan {
  return {
    threadId: row.threadId,
    turnId: row.turnId,
    status: row.status,
...
function latestTaskPlanFromRows(
  rows: ReadonlyArray<ProjectionThreadTaskPlanRow>,
): OrchestrationTaskPlan | null {
  const row = rows.at(-1);
  return row ? mapTaskPlanRow(row) : null;
}
...
function mapActivityMetadataRow(
  row: ProjectionThreadActivityMetadataRow,
...
function mapCheckpointRow(row: ProjectionCheckpointRow): OrchestrationCheckpointSummary {
  return {
    turnId: row.turnId,
...
function buildThreadDetail(input: {
  readonly threadRow: ProjectionThreadRow;
  readonly messages: ReadonlyArray<OrchestrationMessage>;
  readonly proposedPlans: ReadonlyArray<OrchestrationProposedPlan>;
  readonly latestTaskPlan: OrchestrationTaskPlan | null;
  readonly activities: ReadonlyArray<OrchestrationThreadActivity>;
  readonly checkpoints: ReadonlyArray<OrchestrationCheckpointSummary>;
  readonly latestTurnRow: Option.Option<ProjectionLatestTurnRow>;
  readonly sessionRow: Option.Option<ProjectionThreadSessionRow>;
}): OrchestrationThread {
  ...
  return {
    ...
    latestTaskPlan: input.latestTaskPlan,
...
const makeProjectionSnapshotQuery = Effect.gen(function* () {
  ...
  const listThreadTaskPlanRows = SqlSchema.findAll({
    Request: Schema.Void,
    Result: ProjectionThreadTaskPlanDbRowSchema,
    execute: () =>
      sql`
        SELECT
          thread_id AS "threadId",
        ...
        ORDER BY thread_id ASC, updated_at ASC, turn_id ASC
      `,
  });
  ...
  const listLatestThreadMessageRowsByThread = SqlSchema.findAll({
    Request: ThreadDetailLimitedLookupInput,
    Result: ProjectionThreadMessageDbRowSchema,
  ...
  const listThreadTaskPlanRowsByThread = SqlSchema.findAll({
    Request: ThreadIdLookupInput,
    Result: ProjectionThreadTaskPlanDbRowSchema,
    execute: ({ threadId }) =>
      sql`
  ...
  const listLatestThreadActivityMetadataRowsByThread = SqlSchema.findAll({
    Request: ThreadDetailLimitedLookupInput,
    Result: ProjectionThreadActivityMetadataDbRowSchema,
    execute: ({ threadId, limit }) =>
      sql`
        SELECT *
        FROM (
    ...
    sql
      .withTransaction(
        Effect.all([
          ...
          listThreadTaskPlanRows(undefined).pipe(
            Effect.mapError(
              toPersistenceSqlOrDecodeError(
                "ProjectionSnapshotQuery.getSnapshot:listThreadTaskPlans:query",
                "ProjectionSnapshotQuery.getSnapshot:listThreadTaskPlans:decodeRows",
              ),
            ),
          ),
        ...
        Effect.flatMap(
          ([
            ...
            Effect.gen(function* () {
              ...
              const latestTaskPlanByThread = new Map<string, OrchestrationTaskPlan>();
              ...
              for (const row of taskPlanRows) {
                updatedAt = maxIso(updatedAt, row.updatedAt);
                latestTaskPlanByThread.set(row.threadId, mapTaskPlanRow(row));
              }
              ...
              const threads: ReadonlyArray<OrchestrationThread> = threadRows.map((row) => ({
                ...
                latestTaskPlan: latestTaskPlanByThread.get(row.threadId) ?? null,
      ...
      .withTransaction(
        Effect.all([
          ...
          listThreadTaskPlanRows(undefined).pipe(
            Effect.mapError(
              toPersistenceSqlOrDecodeError(
                "ProjectionSnapshotQuery.getCommandReadModel:listThreadTaskPlans:query",
                "ProjectionSnapshotQuery.getCommandReadModel:listThreadTaskPlans:decodeRows",
              ),
            ),
          ),
          listThreadSessionRows(undefined).pipe(
        ...
        Effect.flatMap(
          ([
            ...
            Effect.sync(() => {
              ...
              for (let index = 0; index < taskPlanRows.length; index += 1) {
                const row = taskPlanRows[index];
                if (!row) {
                  continue;
                }
                updatedAt = maxIso(updatedAt, row.updatedAt);
  ...
  const getThreadDetailById: ProjectionSnapshotQueryShape["getThreadDetailById"] = (threadId) =>
    Effect.gen(function* () {
      const [
        ...
        listThreadTaskPlanRowsByThread({ threadId }).pipe(
          Effect.mapError(
            toPersistenceSqlOrDecodeError(
              "ProjectionSnapshotQuery.getThreadDetailById:listTaskPlans:query",
              "ProjectionSnapshotQuery.getThreadDetailById:listTaskPlans:decodeRows",
            ),
          ),
        ),
      ...
      Effect.gen(function* () {
        const [
          ...
          getActiveThreadRowById({ threadId }).pipe(
            Effect.mapError(
              toPersistenceSqlOrDecodeError(
                "ProjectionSnapshotQuery.getThreadDetailSubscriptionSnapshotById:getThread:query",
                "ProjectionSnapshotQuery.getThreadDetailSubscriptionSnapshotById:getThread:decodeRow",
              ),
            ),
          ),
          listLatestThreadMessageRowsByThread({
            ...
            Effect.mapError(
              toPersistenceSqlOrDecodeError(
                "ProjectionSnapshotQuery.getThreadDetailSubscriptionSnapshotById:listMessages:query",
                "ProjectionSnapshotQuery.getThreadDetailSubscriptionSnapshotById:listMessages:decodeRows",
              ),
            ),
          ),
          listThreadProposedPlanRowsByThread({ threadId }).pipe(
            Effect.mapError(
              toPersistenceSqlOrDecodeError(
                "ProjectionSnapshotQuery.getThreadDetailSubscriptionSnapshotById:listPlans:query",
                "ProjectionSnapshotQuery.getThreadDetailSubscriptionSnapshotById:listPlans:decodeRows",
              ),
            ),
          ),
          listThreadTaskPlanRowsByThread({ threadId }).pipe(
            Effect.mapError(
              toPersistenceSqlOrDecodeError(
                "ProjectionSnapshotQuery.getThreadDetailSubscriptionSnapshotById:listTaskPlans:query",
                "ProjectionSnapshotQuery.getThreadDetailSubscriptionSnapshotById:listTaskPlans:decodeRows",
              ),
            ),
          ),
          listLatestThreadActivityMetadataRowsByThread({
            threadId,
            limit: options.activityLimit,
          }).pipe(
            Effect.mapError(
              toPersistenceSqlOrDecodeError(
                "ProjectionSnapshotQuery.getThreadDetailSubscriptionSnapshotById:listActivities:query",
                "ProjectionSnapshotQuery.getThreadDetailSubscriptionSnapshotById:listActivities:decodeRows",
              ),
            ),
          ),
          listLatestCheckpointRowsByThread({
            ...
            Effect.mapError(
              toPersistenceSqlOrDecodeError(
                "ProjectionSnapshotQuery.getThreadDetailSubscriptionSnapshotById:listCheckpoints:query",
                "ProjectionSnapshotQuery.getThreadDetailSubscriptionSnapshotById:listCheckpoints:decodeRows",
              ),
            ),
          ),
          getLatestTurnRowByThread({ threadId }).pipe(
            Effect.mapError(
              toPersistenceSqlOrDecodeError(
                "ProjectionSnapshotQuery.getThreadDetailSubscriptionSnapshotById:getLatestTurn:query",
                "ProjectionSnapshotQuery.getThreadDetailSubscriptionSnapshotById:getLatestTurn:decodeRow",
              ),
            ),
          ),
          getThreadSessionRowByThread({ threadId }).pipe(
            Effect.mapError(
              toPersistenceSqlOrDecodeError(
                "ProjectionSnapshotQuery.getThreadDetailSubscriptionSnapshotById:getSession:query",
                "ProjectionSnapshotQuery.getThreadDetailSubscriptionSnapshotById:getSession:decodeRow",
              ),
            ),
          ),
        ]);
