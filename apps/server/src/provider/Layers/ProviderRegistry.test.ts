...
import { Effect, Exit, Fiber, Layer, PubSub, Ref, Schema, Scope, Sink, Stream } from "effect";
...
it.layer(Layer.mergeAll(NodeServices.layer, ServerSettingsService.layerTest(), TestHttpClientLive))(
  ...
  (it) => {
    ...
    describe("checkClaudeProviderStatus", () => {
      ...
      it.effect("includes Claude Opus 4.8 with ultracode support on supported versions", () =>
        Effect.gen(function* () {
          const status = yield* checkClaudeProviderStatus(
            defaultClaudeSettings,
            claudeCapabilities(),
          ...
          assert.strictEqual(
            effortDescriptor?.type === "select"
              ? effortDescriptor.options.some((option) => option.id === "ultracode")
              : false,
            true,
          );
        }).pipe(
      ...
      it.effect("hides Claude Opus 4.8 before the required Claude Code version", () =>
        Effect.gen(function* () {
          const status = yield* checkClaudeProviderStatus(
            defaultClaudeSettings,
            claudeCapabilities(),
          ...
          assert.strictEqual(
            status.message,
            "Claude Code v2.1.111 is too old for Claude Opus 4.8. Upgrade to v2.1.154 or newer to access it.",
          );
        }).pipe(
          Effect.provide(
            mockSpawnerLayer((args) => {
