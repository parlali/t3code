...
const makeEmptyRequest = (): Parameters<
...
it.layer(NodeServices.layer)("EnvironmentAuth.layer", (it) => {
  ...
  it.effect("issues administrative first-party browser sessions for local browser policy", () =>
    Effect.gen(function* () {
      const serverAuth = yield* EnvironmentAuth.EnvironmentAuth;

      const exchanged = yield* serverAuth.createFirstPartyBrowserSession(requestMetadata);
      const verified = yield* serverAuth.authenticateHttpRequest(
        makeCookieRequest(exchanged.sessionToken),
      );

      expect(verified.subject).toBe("first-party-browser");
      expect(verified.scopes).toEqual([
        "orchestration:read",
        "orchestration:operate",
        "terminal:operate",
    ...
    }).pipe(Effect.provide(makeEnvironmentAuthLayer({ mode: "web", host: "127.0.0.1" }))),
  );

  it.effect("forbids first-party browser bootstrap for remote-reachable policy", () =>
    Effect.gen(function* () {
      const serverAuth = yield* EnvironmentAuth.EnvironmentAuth;

      const error = yield* serverAuth
        .createFirstPartyBrowserSession(requestMetadata)
        .pipe(Effect.flip);

      expect(error._tag).toBe("ServerAuthForbiddenOperationError");
      if (error._tag === "ServerAuthForbiddenOperationError") {
        expect(error.reason).toBe("first_party_bootstrap_not_allowed");
      }
    }).pipe(Effect.provide(makeEnvironmentAuthLayer({ mode: "web", host: "0.0.0.0" }))),
  );

  it.effect("treats explicit unsafe-no-auth mode as an authenticated administrative session", () =>
    Effect.gen(function* () {
      const serverAuth = yield* EnvironmentAuth.EnvironmentAuth;

      const sessionState = yield* serverAuth.getSessionState(makeEmptyRequest());
      const verified = yield* serverAuth.authenticateHttpRequest(makeEmptyRequest());
