...
export const FIRST_PARTY_BROWSER_SUBJECT = "first-party-browser";
export const UNSAFE_NO_AUTH_SUBJECT = "unsafe-no-auth";
...
export class ServerAuthForbiddenOperationError extends Data.TaggedError(
  ...
  readonly reason: "current_session_revoke_not_allowed" | "first_party_bootstrap_not_allowed";
}> {}
...
export interface EnvironmentAuthShape {
  ...
  >;
  readonly createFirstPartyBrowserSession: (requestMetadata: AuthClientMetadata) => Effect.Effect<
    {
      readonly response: AuthBrowserSessionResult;
      readonly sessionToken: string;
    },
...
export const make = Effect.fn("makeEnvironmentAuth")(function* () {
  ...
  const unsafeNoAuthSession: AuthenticatedSession = {
    sessionId: "unsafe-no-auth" as AuthSessionId,
  ...
  const toBrowserSessionExchangeResult = (session: SessionStore.IssuedSession) =>
    ({
      response: {
        authenticated: true,
  ...
  const getSessionState: EnvironmentAuthShape["getSessionState"] = (request) => {
    ...
    return authenticateRequest(request).pipe(
      Effect.map(
        (session) =>
          ({
            authenticated: true,
  ...
  const createBrowserSession: EnvironmentAuthShape["createBrowserSession"] = (
    ...
    bootstrapCredentials.consume(credential).pipe(
      ...
      Effect.map(toBrowserSessionExchangeResult),
      Effect.withSpan("EnvironmentAuth.createBrowserSession"),
    );

  const createFirstPartyBrowserSession: EnvironmentAuthShape["createFirstPartyBrowserSession"] =
    Effect.fn("EnvironmentAuth.createFirstPartyBrowserSession")(function* (requestMetadata) {
      if (
        descriptor.policy !== "loopback-browser" &&
        descriptor.policy !== "desktop-managed-local"
      ) {
        return yield* new ServerAuthForbiddenOperationError({
          reason: "first_party_bootstrap_not_allowed",
        });
      }
      ...
      return yield* sessions
        .issue({
          method: "browser-session-cookie",
          subject: FIRST_PARTY_BROWSER_SUBJECT,
          scopes: AuthAdministrativeScopes,
        ...
        .pipe(
          ...
          Effect.mapError(
            (cause) =>
              new ServerAuthInternalError({
                message: "Failed to issue first-party browser session.",
                cause,
              }),
          ),
        );
  ...
  return {
    ...
    createFirstPartyBrowserSession,
