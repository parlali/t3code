...
function appendBrowserSessionCookie(input: {
  ...
  return Effect.gen(function* () {
    const sessionCookies = yield* Effect.fromResult(
      Cookies.set(Cookies.empty, input.cookieName, input.sessionToken, {
  ...
  Effect.fnUntraced(function* (handlers) {
    ...
    return handlers
      ...
      .handle(
        ...
        Effect.fn("environment.auth.browserSession")(
          function* (args) {
            ...
            yield* appendBrowserSessionCookie({
      ...
      .handle(
        ...
        Effect.fn("environment.auth.firstPartyBrowserSession")(
          function* (args) {
            yield* annotateEnvironmentRequest(args.endpoint.name);
            const request = yield* HttpServerRequest.HttpServerRequest;
            const result = yield* serverAuth.createFirstPartyBrowserSession(
              deriveAuthClientMetadata({ request }),
            );
            yield* appendBrowserSessionCookie({
              sessionToken: result.sessionToken,
