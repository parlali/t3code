...
export function deriveWorkLogEntries(
  ...
  for (const activity of ordered) {
    if (latestTurnId && activity.turnId && activity.turnId !== latestTurnId) continue;
...
export function hasToolActivityForTurn(
  ...
  return activities.some((activity) => {
    ...
    return activity.kind === "task.progress" || activity.kind === "task.completed";
  });
}
...
export function deriveCompletionDividerBeforeEntryId(
  ...
  const firstWorkEntry = timelineEntries.find(
    (entry) =>
      entry.kind === "work" &&
  ...
  return (
    timelineEntries.find(
      (entry) => entry.kind === "message" && entry.message.id === latestTurn.assistantMessageId,
    )?.id ?? null
  );
}
