...
async function installAuthApi(input: {
  ...
  readonly firstPartyBrowserSession?: () => Effect.Effect<AuthBrowserSessionResult>;
  ...
  const testApi = await installEnvironmentHttpTest({
    ...
    ...(input.browserSession
      ...
      : {}),
    ...(input.firstPartyBrowserSession
      ? { firstPartyBrowserSession: () => input.firstPartyBrowserSession!() }
      : {}),
...
describe("resolveInitialServerAuthGateState", () => {
  ...
  it("uses https urls when the primary environment uses wss", async () => {
    await installAuthApi({ session: () => unauthenticatedSession(REMOTE_AUTH) });
  ...
  it("silently bootstraps local browser sessions without a desktop bridge", async () => {
    const nextSession = sequence(
    ...
    expect(testApi.calls.firstPartyBrowserSession).toBe(1);
    expect(testApi.calls.browserSession).toEqual([]);
    expect(testApi.calls.session).toBe(2);
  });

  it("returns a requires-auth state instead of throwing when remote auth has no bootstrap credential", async () => {
    await installAuthApi({ session: () => unauthenticatedSession(REMOTE_AUTH) });
    const { resolveInitialServerAuthGateState } = await import("./environments/primary");
  ...
  it("retries transient auth session bootstrap failures after restart", async () => {
    ...
    const fetchMock = vi
      ...
      .mockResolvedValueOnce(
        new Response(JSON.stringify(encodeAuthSessionState(unauthenticatedSession(REMOTE_AUTH))), {
          status: 200,
  ...
  it("allows manual token submission after the initial auth check requires pairing", async () => {
    const nextSession = sequence(
      unauthenticatedSession(REMOTE_AUTH),
      authenticatedSession(REMOTE_AUTH),
    );
    const testApi = await installAuthApi({
      session: nextSession,
