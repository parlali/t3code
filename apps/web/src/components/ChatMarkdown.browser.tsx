...
const { mermaidInitializeMock, mermaidRenderMock } = vi.hoisted(() => ({
  ...
  mermaidRenderMock: vi.fn(async () => ({
    svg: '<svg viewBox="0 0 100 40" role="img"><text>Rendered Mermaid</text></svg>',
  })),
}));
...
describe("ChatMarkdown", () => {
  ...
  it("renders wrapping URLs as plain external links", async () => {
    ...
    try {
      ...
      expect(link.element().querySelector(".chat-markdown-link-favicon")).toBeNull();
      expect(link.element().querySelector(".chat-markdown-link-leading")).toBeNull();
      expect(link.element().querySelectorAll("wbr").length).toBe(0);
  ...
  it("renders mermaid code fences as full-width diagrams", async () => {
    ...
    try {
      await vi.waitFor(() => {
        ...
        expect(mermaidRenderMock).toHaveBeenCalledWith(
          expect.stringMatching(/^chat-markdown-mermaid-/),
          "graph TD\n  A --> B",
        );
      });

      const diagram = document.querySelector<HTMLElement>(".chat-markdown-mermaid");
      expect(diagram?.getAttribute("data-render-state")).toBe("rendered");
      expect(diagram?.querySelector("svg")).not.toBeNull();
      expect(getComputedStyle(diagram!).width).not.toBe("auto");
    } finally {
      await screen.unmount();
