...
function GitFetchIntervalSettings() {
  ...
  const automaticGitFetchIntervalSeconds = millisToSeconds(automaticGitFetchInterval);
  const defaultAutomaticGitFetchIntervalSeconds = millisToSeconds(
  ...
  const updateFetchInterval = (value: number) => {
    updateSettings({
      automaticGitFetchInterval: normalizeFetchIntervalSeconds(value) * 1_000,
    });
  };
    ...
    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
      <div className="min-w-0 space-y-1">
        ...
        <p className="max-w-2xl text-xs leading-relaxed text-muted-foreground">
          Refresh remote branch status in the background. Set this to 0 seconds if credentials or
      ...
      <label className="flex shrink-0 items-center gap-2">
        <Input
          ...
          onChange={(event) => updateFetchInterval(event.currentTarget.valueAsNumber)}
